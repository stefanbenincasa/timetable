...
export class PSQLStudentRepository implements StudentRepository {
  async storeNew(student: Student): Promise<Student> {
		const queryRes = await pgPool.query(
			`INSERT INTO student(first_name, last_name, email, password) VALUES(LOWER($1), LOWER($2), $3, $4) 
			RETURNING student_id AS studentId, first_name AS firstName, last_name AS lastName, email, password;`,
	...
	async readStudent(studentId: number): Promise<Student> {
		const queryRes = await pgPool.query('SELECT student_id AS studentId, first_name AS firstName, last_name AS lastName, email, password FROM student WHERE student_id = $1;', [studentId])
